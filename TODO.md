...
- **Fork partial fix**: Added TextMate scope colors for C/C++ `enum` type names and members in the Default variant. Generic variable/parameter bulk-highlighting remains out of scope per design philosophy. Colorful variant already covered by generic `entity.name.type.enum` TextMate rule. See nv-h PR [#3](https://github.com/nv-h/gruvbox-material-vscode/pull/3).
